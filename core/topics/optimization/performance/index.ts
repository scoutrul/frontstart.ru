...
export const WEB_VITALS_TOPICS: Topic[] = [
  ...
  },
  {
    ...
    examples: [
      {
        title: 'Синхронный JS блокирует парсинг',
        code: `// БРАУЗЕР:
// 1. Парсит HTML
// 2. Встречает <script src="chunk1.js"></script>
// 3. ОСТАНАВЛИВАЕТ парсинг HTML
// 4. Загружает chunk1.js
// 5. ВЫПОЛНЯЕТ chunk1.js
// 6. ПРОДОЛЖАЕТ парсинг HTML
// 7. Встречает <script src="chunk2.js"></script>
// 8. ОСТАНАВЛИВАЕТ парсинг снова
// 9. Загружает chunk2.js
// 10. ВЫПОЛНЯЕТ chunk2.js
// 11. ПРОДОЛЖАЕТ парсинг

// ВАЖНО: Каждый чанк блокирует отдельно!
// Браузер НЕ ждёт загрузки всех чанков сразу`
      },
      {
        title: 'CSS чанки блокируют рендер по отдельности',
        code: `// БРАУЗЕР:
// 1. Парсит HTML
// 2. Встречает <link rel="stylesheet" href="styles.css">
// 3. ОСТАНАВЛИВАЕТ рендер
// 4. Загружает styles.css
// 5. ПРИМЕНЯЕТ стили
// 6. ПРОДОЛЖАЕТ рендер
// 7. Встречает <link rel="stylesheet" href="components.css">
// 8. ОСТАНАВЛИВАЕТ рендер снова
// 9. Загружает components.css
// 10. ПРИМЕНЯЕТ стили
// 11. ПРОДОЛЖАЕТ рендер

// ВАЖНО: Каждый CSS-файл блокирует отдельно!
// Браузер НЕ ждёт загрузки всех CSS-файлов сразу`
      },
      {
        title: 'Async убирает parser-blocking',
        code: `// БРАУЗЕР:
// 1. Парсит HTML
// 2. Встречает <script src="analytics.js" async></script>
// 3. ПРОДОЛЖАЕТ парсинг HTML (не останавливается!)
// 4. Загружает analytics.js параллельно
// 5. После загрузки выполняется (может прервать парсинг)

// ПОВЕДЕНИЕ:
// - Парсинг НЕ блокируется
// - Загрузка параллельная
// - Выполнение после загрузки (может прервать парсинг)
...
// - Виджеты`
      ...
      {
        title: 'Defer убирает parser-blocking и гарантирует порядок',
        code: `// БРАУЗЕР:
// 1. Парсит HTML
// 2. Встречает <script src="vendor.js" defer></script>
// 3. ПРОДОЛЖАЕТ парсинг HTML (не останавливается!)
// 4. Загружает vendor.js параллельно
// 5. Встречает <script src="app.js" defer></script>
// 6. ПРОДОЛЖАЕТ парсинг HTML
// 7. Загружает app.js параллельно
// 8. После парсинга HTML выполняется vendor.js
// 9. После vendor.js выполняется app.js
...
// - Когда важен порядок выполнения`
      ...
      {
        title: 'Async/defer не убирают render-blocking полностью',
        code: `// ПРОБЛЕМА:
// Даже с async/defer скрипт может задержать рендер

<script src="heavy-script.js" defer></script>

// Если heavy-script.js:
// - Изменяет DOM до завершения рендера
// - Загружает критичные стили
// - Выполняет тяжёлые вычисления

// → Рендер всё равно задерживается
